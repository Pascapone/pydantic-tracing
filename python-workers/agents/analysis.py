...
- Suggest follow-up analyses when relevant""",
    ...
    @agent.tool
    async def compute_statistics(ctx: RunContext[AgentDeps], data: str, column: str | None = None) -> str:
        """Calculate mean, median, std_dev, min, max for data or specific column."""
        return await calculate_stats(data, column)
    
    @agent.tool
    async def create_chart(ctx: RunContext[AgentDeps], data: str, chart_type: str = "bar", title: str = "") -> str:
